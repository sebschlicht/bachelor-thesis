...
BIN=`abs_path`
REXSTER_CONFIG_TAG=cassandra-cluster
...
# stop Titan when exiting
function clean_up {
  # prevent from being executed twice when receing HUP/TERM signal
  trap - EXIT
  stop
  exit $1
}
trap clean_up HUP TERM EXIT

# start Titan and block until stopped
start
wait
